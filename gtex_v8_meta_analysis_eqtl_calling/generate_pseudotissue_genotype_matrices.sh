...
for chrom_num in $(seq 1 22); do 
	...
	plink --bfile ${gtex_genotype_dir}"GTEx_v8_genotype_EUR_1kg_overlap_maf_05_"${chrom_num} --keep ${pseudotissue_individual_names} --indiv-sort f ${pseudotissue_individual_names} --make-bed --out ${pseudotissue_genotype_dir}${pseudotissue_name}"_GTEx_v8_genotype_EUR_"${chrom_num}
	
	# Get Allele frequency of the snps in this tissue
	plink --bfile ${pseudotissue_genotype_dir}${pseudotissue_name}"_GTEx_v8_genotype_EUR_"${chrom_num} --freq --out ${pseudotissue_genotype_dir}${pseudotissue_name}"_GTEx_v8_genotype_EUR_"${chrom_num}
	...
	plink --bfile ${pseudotissue_genotype_dir}${pseudotissue_name}"_GTEx_v8_genotype_EUR_"${chrom_num} --recode vcf --out ${pseudotissue_genotype_dir}${pseudotissue_name}"_GTEx_v8_genotype_EUR_"${chrom_num}
