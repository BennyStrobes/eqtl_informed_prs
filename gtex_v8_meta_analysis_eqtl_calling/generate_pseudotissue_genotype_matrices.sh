...
for chrom_num in $(seq 1 22); do 
	echo $chrom_num
	plink --bfile ${gtex_genotype_dir}"GTEx_v8_genotype_EUR."${chrom_num} --keep ${pseudotissue_individual_names} --geno 0.0 --maf .05 --recode vcf --out ${pseudotissue_genotype_dir}${pseudotissue_name}"_GTEx_v8_genotype_EUR_"${chrom_num}


	python3 convert_vcf_file_to_dosage_file.py ${pseudotissue_genotype_dir}${pseudotissue_name}"_GTEx_v8_genotype_EUR_"${chrom_num}
	python3 reorder_vcf_dosage_columns.py ${pseudotissue_genotype_dir}${pseudotissue_name}"_GTEx_v8_genotype_EUR_"${chrom_num} $pseudotissue_individual_names
