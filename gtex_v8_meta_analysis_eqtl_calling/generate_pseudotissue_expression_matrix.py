...
def get_normalized_expression(tissue, pseudotissue_ordered_sample_names, gtex_expression_dir, normalized_expression_matrix_file):
	...
	for line in f:
		...
		if head_count == 0:
			...
			t.write('SampleId' + '\t' + '\t'.join(line_samples[mapping]) + '\n')
			continue
		ensamble_id = data[3]
		line_samples = np.asarray(data[4:])
		...
		t.write(ensamble_id + '\t' + '\t'.join(norm_expr.astype(str)) + '\n')
	f.close()
	t.close()
...
tissue = sys.argv[1]
pseudotissue_sample_names_dir = sys.argv[2]
gtex_normalized_expression_dir = sys.argv[3]
gene_annotation_file = sys.argv[4]
pseudotissue_expression_dir = sys.argv[5]
...
pseudotissue_ordered_sample_names = get_pseudotissue_sample_names(pseudotissue_sample_names_dir + tissue + '_individual_names.txt')


# Quantile normalize and standardize TPM expression matrix
normalized_expression_matrix_file = pseudotissue_expression_dir + tissue + '_normalized_expression.txt'
get_normalized_expression(tissue, pseudotissue_ordered_sample_names, gtex_normalized_expression_dir, normalized_expression_matrix_file)


# Filter genes based on autosomes and protein-coding linc
normalized_expression_matrix_filtered_genes_file = pseudotissue_expression_dir + tissue + '_normalized_expression_autosomes_protein_coding_and_linc.txt'
filter_expression_matrix(normalized_expression_matrix_file, normalized_expression_matrix_filtered_genes_file, gene_name_to_gene_info)
