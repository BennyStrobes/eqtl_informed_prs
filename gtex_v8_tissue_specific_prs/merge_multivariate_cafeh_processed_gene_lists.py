...
tissue_merged_file = processed_bivariate_cafeh_input_dir + trait_name + '_processed_gene_list.txt'
t = open(tissue_merged_file, 'w')
chrom_counter = 0
for chrom_num in range(1,23):
	input_file = processed_bivariate_cafeh_input_dir + trait_name + '_chr' + str(chrom_num) + '_processed_gene_list.txt'
	f = open(input_file)
	head_count = 0
	for line in f:
		line = line.rstrip()
