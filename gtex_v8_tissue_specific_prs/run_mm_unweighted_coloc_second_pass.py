...
def run_causal_coloc_for_single_gene(tissue_names, tissue_name_to_position, gene_name, causal_prior, beta_file, snp_pph4_file, gene_pph4_file, coloc_output_dir, trait_name, causal_version):
	...
	if causal_version == 'v1':
		gene_posteriors = coloc.get_causal_coloc_prob_from_pph4_vec_with_prior(pph4_vec, causal_prior)
	...
	for eqtl_study_num, eqtl_study_name in enumerate(tissue_names):
		...
		for threshold_iter, coloc_threshold in enumerate(coloc_thresholds):
			if pph4_vec[global_tissue_position]*gene_posteriors[global_tissue_position] > coloc_threshold:
				...
				predicted_effects_list[threshold_iter][:, global_tissue_position] = snp_pph4[global_tissue_position,:]*beta_vec

	# Save predicted effects mat
	for threshold_iter, coloc_threshold in enumerate(coloc_thresholds):
		if coloc_at_threshold_arr[threshold_iter]:
			predicted_effect_size_file = coloc_output_dir + trait_name + '_' + gene_name + '_mm_' + causal_version + '_coloc_' + str(coloc_threshold) + '_predicted_effect_sizes.txt'
	...
	# Save pph mat
	pph_mat_output_file = coloc_output_dir + trait_name + '_' + gene_name + '_mm_' + causal_version + '_coloc_posterior_probabilities.txt'
...
def get_causal_posteriors(pph4_mat, causal_prior, causal_version):
	num_genes = pph4_mat.shape[0]
	causal_posteriors_arr = []
	for gene_num in range(num_genes):
		if causal_version == 'v1':
			gene_posteriors = coloc.get_causal_coloc_prob_from_pph4_vec_with_prior(pph4_mat[gene_num,:], causal_prior)
		elif causal_version == 'v2':
			...
			pdb.set_trace()
		causal_posteriors_arr.append(gene_posteriors*np.max(pph4_mat[gene_num,:]))
	return np.asarray(causal_posteriors_arr)

def run_iterative_algorithm_to_get_causal_prior(pph4_mat, causal_version, pseudocount=1e-9, max_iters=100):
	num_dim = pph4_mat.shape[1]
	# Initialize causal prior
	causal_prior = np.zeros(num_dim) + 1.0/num_dim

	for itera in range(max_iters):
		causal_posteriors = get_causal_posteriors(pph4_mat, causal_prior, causal_version)
...
causal_prior = run_iterative_algorithm_to_get_causal_prior(pph4_mat, causal_version)


t = open(coloc_output_dir + trait_name + '_coloc_mm_' + causal_version + '_causal_prior_probability.txt','w')
...
for gene_num in range(num_genes):
	...
	gene_pph4_file = coloc_output_dir + trait_name + '_' + gene_name + '_coloc_posterior_probabilities.txt'
	run_causal_coloc_for_single_gene(tissue_names, tissue_name_to_position, gene_name, causal_prior, beta_file, snp_pph4_file, gene_pph4_file, coloc_output_dir, trait_name, causal_version)
