...
run_susie_with_betas <- function(trait_data) {
	prior_variance=50
	estimate_prior_variance=TRUE
	check_prior=TRUE
	res = susie_suff_stat(bhat=trait_data$beta, shat=sqrt(trait_data$varbeta),R=trait_data$LD, n=trait_data$N, scaled_prior_variance=prior_variance, estimate_prior_variance=estimate_prior_variance, residual_variance=1, estimate_residual_variance=FALSE, standardize=FALSE, check_prior=check_prior)
	snp = trait_data$snp
	colnames(res$lbf_variable) = c(trait_data$snp,"null")[1:ncol(res$lbf_variable)]
	colnames(res$alpha) = c(trait_data$snp,"null")[1:ncol(res$alpha)]
 	names(res$pip)=trait_data$snp
  if(length(res$sets$cs))
    res$sets$cs = lapply(res$sets$cs, function(x) { names(x) = snp[x]; x })
  res$sld=.susie_setld(res$sets$cs,trait_data$LD)
  res$pruned=FALSE
  return(res)
}
...
run_susie_coloc_for_single_gene <- function(gene_name, genotype_file, n_file, beta_file, std_err_file, tissue_names, coloc_output_dir) {
	...
	trait_data_susie = run_susie_with_betas(trait_data)
	#trait_data_susie=runsusie(trait_data)
	saveRDS(trait_data_susie, file=paste0(gene_name,"_trait_data_susie.RDS"))
	num_trait_components = length(trait_data_susie$sets$cs)
	...
	if (num_trait_components > 0) {
		...
		for (eqtl_study_num in 1:length(eqtl_studies)) {
			...
			num_eqtl_components = length(eqtl_data_susie$sets$cs)
			print(num_eqtl_components)
			if (num_eqtl_components > 0) {
				...
				if (max(res$summary[,8]) > .5) {
					...
					saveRDS(trait_beta, file=paste0(gene_name,"_", eqtl_study_name, "_trait_beta.RDS"))
					saveRDS(trait_var_beta, file=paste0(gene_name,"_", eqtl_study_name, "_trait_var_beta.RDS"))
					saveRDS(LD, file=paste0(gene_name,"_", eqtl_study_name, "_ld.RDS"))
				}
			}
		}
