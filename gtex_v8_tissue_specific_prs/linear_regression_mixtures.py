...
class LinearRegressionsMixture(Model):
	...
	def _expectation_maximization(self, X, y, verbose):
		...
		for i in range(self.iterations):
			...
			print(str(i) + '\t' + str(complete_log_likelihood))
			...
			try:
				if np.isnan(complete_log_likelihood) \
						...
						or np.abs(complete_log_likelihood - complete_log_likelihood_old) < self.epsilon:
					return w, pi, gamma, beta, complete_log_likelihood, gamma
			except:
				pass
	...
	def _train(self, X, y, verbose, silent):
		...
		if self.random_restarts and self.random_restarts > 0:
			w = pi = gamma = beta = probabilities = None
			...
			for r in range(self.random_restarts):
				w_new, pi_new, gamma_new, beta_new, marginal_likelihood_new, probabilities_new = self._expectation_maximization(X, y, verbose)
			...
			w, pi, gamma, beta, marginal_likelihood, probabilities = self._expectation_maximization(X, y, verbose)

		return w, pi, gamma, beta, marginal_likelihood, probabilities
	...
	def train(self, seed=None, verbose=False, silent=False, **kwargs):
		...
		if seed:
			np.random.seed(seed)
		self.w, self.pi, self.gamma, self.beta, self.marginal_likelihood, self.probability = self._train(self.X, self.y, verbose, silent)
		self.trained = True
