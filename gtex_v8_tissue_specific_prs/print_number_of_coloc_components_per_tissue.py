...
for coloc_threshold in coloc_thresholds:
	...
	for chrom_num in range(1,23):
		component_file = bivariate_cafeh_output_dir + 'adaptive_prior_coloc_results_' + trait_name + '_' + str(coloc_threshold) + '_num_prs_components_chrom_' + str(chrom_num) + '.txt'
	...
	# print to output file
	t = open(bivariate_cafeh_output_dir + 'adaptive_prior_coloc_results_' + trait_name+ '_' + str(coloc_threshold) + '_num_prs_components.txt', 'w')
	t.write('tissue\tsample_size\tnum_cafeh_components\tnum_eqtl_components\tavg_num_expressed_genes\n')
	for tissue in tissues:
		samp_size = tissue_to_sample_size[tissue]
		num_comp = tissue_to_num_components[tissue]
